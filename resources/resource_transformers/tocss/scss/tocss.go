...
import (
	...
	sasscompiler "github.com/wellington/sass/compiler"
)
...
func (t *toCSSTransformation) Transform(ctx *resources.ResourceTransformationCtx) error {
	...
	*/
	...
	return t.c.toCSS(t.options, ctx.To, ctx.From)

	/*
		res, err := t.c.toCSS(options, ctx.To, ctx.From)
		if err != nil {
			return err
		}
...
func (c *Client) toCSS(options Options, dst io.Writer, src io.Reader) error {
	b, err := ioutil.ReadAll(src)
	if err != nil {
		return err
	}

	res, err := sasscompiler.Compile(b)
	if err != nil {
		return err
	}
